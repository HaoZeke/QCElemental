...
def write_c_header(context, filename="physconst.h", prefix="pc_"):
    """Write C header file defining physical constants and pi, constants names are prefixed."""
    ...
    text = [
        ...
        "",
        "/* clang-format off */",
        f"#define {prefix}pi {pi}",
        f"#define {prefix}twopi {tau}",
    ]
...
def write_fortran_header(context, filename="physconst.fh", prefix="pc_", kind=None):
    """Write Fortran include file defining physical constants and pi, constants names are prefixed.
    The name of the kind parameter can be passed as well."""
